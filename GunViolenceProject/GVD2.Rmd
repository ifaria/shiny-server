...
#####
Data <- Data %>%
  select(Incident, Date, Name, Street, City, Longitude, Latitude, Notes, States, Counties, Num_Participiants, Injury)
...
Data_Incident_Characteristics - map_dfc(Data_Incident_Characteristics, as.factor)

saveRDS(Data_Incident_Characteristics, "Data_Incident_Characteristics.rds")
...
Data_Guns <- map_dfc(Data_Guns, as.factor)
saveRDS(Data_Guns, "Data_Guns.rds")
...
Data_Participants <- map_dfc(Data_Participants, as.factor)
saveRDS(Data_Participants, "Data_Participants.rds")
...
### Incidents per State
iepattern = one_or_more(START %R% ", ") %R% ", "

Data_sum <- Data %>%
  mutate(States = ifelse(is.na(States), str_remove(City, pattern = iepattern), States)) %>%
  dplyr::group_by(States) %>%
  count()

Data_sum$States <- str_to_lower(Data_sum$States)
saveRDS(Data_sum, "Data_sum.rds")
names(States)
